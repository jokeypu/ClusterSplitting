int pi0_mom(){
    ...
    TH2D* h_E = new TH2D("Hist_E","h_E",nbins2,0,xmax_cp,(int)(nbins2*0.75),0,1);
    h_E->SetMarkerStyle(7);
    h_E->SetMarkerColorAlpha(kAzure+3, 0.5);
    h_E->GetXaxis()->SetTitle("P_{#pi^{0}}   [GeV/c]");
    h_E->GetYaxis()->SetTitle("E_{#gamma min}/E_{#gamma max}");
    ...
    for (double E_pi0 = step/2.0; E_pi0 < 25 ; E_pi0+= step){
        for (int i = 0; i < NN ; i++){
            ...
            h->Fill(sqrt(E_pi0*E_pi0-m_pi0*m_pi0),delta_angle);
            //h_E->Fill(sqrt(E_pi0*E_pi0-m_pi0*m_pi0), E_gamma1);
            //h_E->Fill(sqrt(E_pi0*E_pi0-m_pi0*m_pi0), E_gamma2);
            h_E->Fill(sqrt(E_pi0*E_pi0-m_pi0*m_pi0), E_gamma1<E_gamma2? E_gamma1/E_gamma2 : E_gamma2/E_gamma1);
        }
    }
